...
from langchain import Modal, LLMChain
...
from modelscope import AutoModelForCausalLM, GenerationConfig, AutoTokenizer
...
if __name__ == "__main__":
    ...
    # llm = AutoModelForCausalLM.from_pretrained("qwen/Qwen-7B-Chat", device_map='cpu', low_cpu_mem_usage=True, trust_remote_code=True).eval()
    # llm.generation_config = GenerationConfig.from_pretrained("Qwen/Qwen-7B-Chat",  trust_remote_code=True)
    # llm.float()
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # llm.to(device)
    # llm = Modal(model_name="qwen/Qwen-7B-Chat")
    ...
    response = agent_executor.run("介绍下Rimix这个产品?")
